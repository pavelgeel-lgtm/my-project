...
export default function InviteScreen({ onActivated }) {
  ...
  return (
    <div style={{ background:'#0e0e0e', minHeight:'100vh', display:'flex', flexDirection:'column', fontFamily:'var(--font)', position:'relative', overflow:'hidden' }}>
      {/* Scanline */}
      <div style={{ position:'absolute', inset:0, backgroundImage:'repeating-linear-gradient(0deg,rgba(255,255,255,0.012) 0px,rgba(255,255,255,0.012) 1px,transparent 1px,transparent 3px)', pointerEvents:'none', zIndex:0 }} />
      ...
      <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:'3rem 2rem', position:'relative', zIndex:2 }}>
        <div style={{ maxWidth:480, width:'100%' }}>
          ...
          {/* Terminal */}
          {phase !== 'idle' && (
            <div style={{ marginTop:16, padding:'16px 20px', background:'rgba(0,0,0,0.4)', border:'1px solid rgba(255,255,255,0.06)' }}>
              {STEPS.map((step, i) => {
                const isDone = doneSteps.includes(i)
                const isActive = activeStep === i
                return (
                  <div key={i} style={{ display:'flex', alignItems:'center', gap:12, padding:'5px 0' }}>
                    <div style={{ width:16, height:16, borderRadius:'50%', border:`1px solid ${isDone ? '#1d9e75' : isActive ? '#c8a84b' : 'rgba(255,255,255,0.15)'}`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, transition:'all 0.3s' }}>
                      <div style={{ width:5, height:5, borderRadius:'50%', background: isDone ? '#1d9e75' : isActive ? '#c8a84b' : 'rgba(255,255,255,0.15)', transition:'all 0.3s' }} />
                    </div>
                    <span style={{ fontSize:12, fontWeight:300, letterSpacing:'0.08em', color: isDone ? '#1d9e75' : isActive ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.2)', transition:'color 0.3s' }}>{step}</span>
                  </div>
                )
              })}
            </div>
          ...
          {/* Welcome */}
          {phase === 'done' && (
            <div style={{ marginTop:16, padding:'16px 20px', borderLeft:'2px solid #1d9e75', background:'rgba(29,158,117,0.07)', display:'flex', alignItems:'center', gap:12 }}>
              <span style={{ fontSize:15, fontWeight:300, color:'#1d9e75', letterSpacing:'0.04em' }}>Добро пожаловать в клуб, Максим Владимирович</span>
            </div>
          )}
        </div>
      </div>
